...
void transfer_orbit(){
    ...
    int a = 27000;
    int e = 0.6;
    int f = deg2rad(30); // ITS FUCKING MATLAB BAYBEEEEEE
    double r = (a*(1-e^2))/(1+e*cos(f));

    // Departure Location:
    int d_a = 8000;
    int d_e = 0.01;
    int d_f = deg2rad(30); // ITS FUCKING MATLAB BAYBEEEEEE
    double d_r = (d_a*(1-d_e^2))/(1+d_e*cos(d_f));

    // Transfer Orbit:
    double delf = abs(d_f - f);
    double aMin = 0.25 * (d_r + r + sqrt(pow(d_r,2) + pow(r,2) - 2 * r * d_r * cos(delf)));
    ...
    vector <double> d_orbit = transform(d_delf, [d_a,d_e](auto x) { return (d_a*(1-pow(d_e,2)))/(1+d_e*cos(x));; });
    vector <double> a_orbit = transform(d_delf, [a,e](auto x) { return (a*(1-pow(e,2)))/(1+e*cos(x));; });
    vector <double> t_orbit = transform(d_delf, [aMin,t_e](auto x) { return (aMin*(1-pow(t_e,2)))/(1+t_e*cos(x));; });
